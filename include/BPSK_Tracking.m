...
function [trackingResults] = BPSK_Tracking(InterFreqSignal,Detect_settings,Interference_Info)
% 定义剩余码相位
...
% finalSignal = zeros(size(InterFreqSignal));
...
%         (sqrt(I_E_Interf * I_E_Interf + Q_E_Interf * Q_E_Interf) + sqrt(I_L_Interf * I_L_Interf + Q_L_Interf * Q_L_Interf));
    ...
    trackingResults(tracking_i, :) = [CodePhaseErr, CarrPhaseErr, rmsAmplitude, I_P, Q_P, blksize, remCodePhase, remCarrPhase, codeFreq, CarrierFreq, CodePhaseErr_Interf,CarrPhaseErr_Interf,N_data(tracking_i)];
